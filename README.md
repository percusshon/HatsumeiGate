...
- ウイルススキャン基盤（`invention_files.scan_status` + 配信ゲートで `infected` をブロック、後方互換）実装済み。実スキャナワーカーは未配線（`docs/virus-scan-proposal.md`）
- 残（コード外・本番運用系）: 本番 Supabase/Vercel 接続・監視・バックアップ、法務文言の最終確定、ウイルススキャンの実スキャナ方式選定・ワーカー配線。詳細は `docs/implementation-roadmap.md` と各 `docs/*-mvp.md` 参照

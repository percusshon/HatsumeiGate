...
- 実装済み: Supabase スキーマ/RLS/storage/seed、Magic Link 認証、inventor の下書き〜提出、operator の審査案件閲覧・ステータス更新（内部審査フェーズ）
- 未実装: 審査レポート、企業開示/NDA、ファイル/signed URL、deal パイプライン（`docs/implementation-roadmap.md` 参照）
...
1. operator 審査レポート（screening / 先行技術 / 知財方針ノート）
2. 企業アカウント / NDA / 開示レベル制御（NDA前後分離）
3. ファイルアップロード + signed URL（開示レベル連動）
4. deal パイプライン
5. audit_logs 連動の検証
